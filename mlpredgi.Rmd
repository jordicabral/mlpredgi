---
title: "PEC3 - TFM - Master Bioinformática y Bioestadística - UOC"
author: "Jordi Cabral"
date: "18/11/2019"
output:
  word_document:
...
Comprobación de los ficheros existentes en el directorio de datos
```{r}
dir(path = dataDir)
```
...
Exploración y visualización superficial de los datos, mostrando su estructura, los primeros registros, y un resumen global
...
Eliminación de las variables identificativas de los genes individuales
```{r}
allpaired$V1 <- NULL
allpaired$V2 <- NULL
```

Conversión a tipo numérico de todas las variables del dataset *allpaireid*
```{r}
allpaired[] <- lapply(allpaired, as.numeric)
str(allpaired)
```

Generación de dos nuevos data frames filtrando sólo los pares existentes en los SGAs
```{r}
allpaired_10 <- allpaired[row.names(allpaired) %in% row.names(SGA10), ]
dim(allpaired_10)
```

Se observa que en lugar de 227 registros hay 190, debido a que algunos genes han cambiado de identificador

```{r}
allpaired_16 <- allpaired[row.names(allpaired) %in% row.names(SGA16), ]
dim(allpaired_16)
```
Se observa que en lugar de 757 registros, hay 635, debido a que algunos genes han cambiado de identificador


Generación de un nuevo dataset, uniendo los campos de SGA y allpaired, para cada uno de los experimientos
```{r}
...
Eliminación de las variables "descriptivas" de los genes
```{r}
...
```
### Modelo de referencia de regresión lineal
Reproducción del modelo de regresión lineal de referencia de la bibliografía, para verificar la base correcta de partida
Modelo 1 (Tabla 1)
```{r}
...
Preparación de los datos tanto para el dataset *SGA10* como *SGA16*, pero inicialmente se realizan los cálculos sólo sobre los datos de *SGA10*, para simplificar la comparación, ya que es sobre el que se tiene la referencia de regresión lineal.

### Selección de datos para *training* y *test*
División de los dataset en dos, para entrenamiento y validación.
```{r}
...
Se genera una tabla para poder visualizar un resumen de las diferentes combinaciones:
```{r}
...
Cambio de alguno de los parámetros por defecto
```{r}
set.seed(seed)
...
Entrenamiento del modelo con el *kernel* lineal (`vanilladot`)
```{r}
set.seed(seed)
...
Evaluación el rendimiento del modelo
```{r}
...
Prueba de mejora del rendimiento del modelo cambiando el tipo de *kernel* que utiliza el modelo.
...
Se muestra un gráfico del modelo
```{r}
...
Prueba de mejora del rendimiento del modelo mediante el cambio de alguno de los  parámetro por defecto.
```{r}
set.seed(seed)
...
Se observa que el comportamiento es bastante estable a partir de 100 árboles aproximádamente.
...
Se observa por ejemplo que la variable *NPC* gana importancia respecto *PCC*.
...
Prueba de mejora del rendimiento del modelo incrementando el número de árboles.
```{r}
set.seed(seed)
...
Se observa que el modelo no ha mejorado aumentando el número de árboles del bosque a 1.000, como se intuía en el gráfico anterior.
...
## Recapitulación del resumen de todos los modelos
Regresión lineal
```{r}
RMSE.lm1
```
...
Se observa que ninguno mejora el valor del modelo de regresión lineal.
El que ha obtenido el mejor valor es el algoritmo Arbol de Decisión, parametrizado, y con la variable NPC.
Si se realizan los cálculos con el dataset SGA16 y esos parámetros:
```{r}
set.seed(seed)
...
Se observa que aunque se incremente el número de observaciones, la predicción no mejora.

Aunque si evaluamos el rendimiento del modelo de regresión lineal con el conjunto de datos *SGA16*:
```{r}
...
```
Comprobamos que es peor también, y el comportamiento equiparable entre ambos modelos y conjuntos de datos.
